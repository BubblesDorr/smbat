...
namespace planning {
ErrorType OnLaneForwardSimulation::GetTargetStateOnTargetLane(
    ...
    if (has_front && has_rear) {
        ...
        desired_v =
            std::min(std::max(v_ref_rear, ego_desired_vel), v_ref_front);
...
ErrorType OnLaneForwardSimulation::PropagateOnceAdvancedLK(
    const common::StateTransformer& stf, const common::Vehicle& ego_vehicle,
    const common::Vehicle& leading_vehicle, const decimal_t& lat_track_offset,
    const decimal_t& dt, const Param& param, common::State* desired_state) {
    ...
    if (!steer_calculation_failed) {
        decimal_t approx_lookahead_dist = std::min(
            std::max(param.steer_control_min_lookahead_dist,
...
ErrorType OnLaneForwardSimulation::PropagateOnceAdvancedLC(
    const common::StateTransformer& stf_current,
    const common::StateTransformer& stf_target,
    const common::Vehicle& ego_vehicle,
    const common::Vehicle& current_leading_vehicle,
    const common::Vehicle& gap_front_vehicle,
    const common::Vehicle& gap_rear_vehicle, const decimal_t& lat_track_offset,
    const decimal_t& dt, const Param& param, common::State* desired_state) {
    common::State current_state = ego_vehicle.state();
    decimal_t wheelbase_len = ego_vehicle.param().wheel_base();
    auto sim_param = param;
    ...
    if (current_leading_vehicle.id() == kInvalidAgentId ||
        ...
        GetIdmEquivalentVehicleLength(stf_current, ego_vehicle,
                                      ...
                                      current_leading_fs, &eqv_vehicle_len);
        sim_param.idm_param.kVehicleLength = eqv_vehicle_len;

        CalcualateVelocityUsingCtxIdm(
            ego_on_tarlane_fs.vec_s[0], current_state.velocity,
            current_leading_fs.vec_s[0],
            current_leading_vehicle.state().velocity,
            target_on_curlane_fs.vec_s[0], target_state.velocity, dt, sim_param,
            ctx_param, &velocity);
    }
...
ErrorType OnLaneForwardSimulation::PropagateOnce(
    const common::StateTransformer& stf, const common::Vehicle& ego_vehicle,
    const common::Vehicle& leading_vehicle, const decimal_t& dt,
...
ErrorType OnLaneForwardSimulation::PropagateOnce(
    const decimal_t& desired_vel, const common::Vehicle& ego_vehicle,
    const decimal_t& dt, const Param& param, common::State* desired_state) {
    common::State current_state = ego_vehicle.state();
    decimal_t wheelbase_len = ego_vehicle.param().wheel_base();
...
ErrorType OnLaneForwardSimulation::GetIdmEquivalentVehicleLength(
    const common::StateTransformer& stf, const common::Vehicle& ego_vehicle,
...
ErrorType OnLaneForwardSimulation::CalcualateSteer(
    const common::StateTransformer& stf, const common::State& current_state,
    const common::FrenetState& current_fs, const decimal_t& wheelbase_len,
    const Vec2f& lookahead_offset, decimal_t* steer) {
...
ErrorType OnLaneForwardSimulation::CalcualateVelocityUsingIdm(
...
ErrorType OnLaneForwardSimulation::CalcualateVelocityUsingIdm(
    const decimal_t& current_vel, const decimal_t& dt, const Param& param,
    decimal_t* velocity) {
...
ErrorType OnLaneForwardSimulation::CalcualateVelocityUsingCtxIdm(
    const decimal_t& current_pos, const decimal_t& current_vel,
    const decimal_t& leading_pos, const decimal_t& leading_vel,
...
ErrorType OnLaneForwardSimulation::CalculateDesiredState(
    const common::State& current_state, const decimal_t steer,
    const decimal_t velocity, const decimal_t wheelbase_len, const decimal_t dt,
