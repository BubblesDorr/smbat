#ifndef _CORE_SEMANTIC_MAP_MANAGER_INCLUDE_SEMANTIC_MAP_MANAGER_SEMANTIC_MAP_MANAGER_H_
#define _CORE_SEMANTIC_MAP_MANAGER_INCLUDE_SEMANTIC_MAP_MANAGER_SEMANTIC_MAP_MANAGER_H_
...
class SemanticMapManager {
  public:
    ...
    ErrorType
    CheckCollisionUsingStateVec(const vec_E<common::State> state_vec) const;
    ...
    ErrorType
    UpdateSemanticMap(const double& time_stamp,
    ...
    ErrorType
    MobilRuleBasedBehaviorPrediction(const common::Vehicle& vehicle,
                                     const common::VehicleSet& nearby_vehicles,
    ...
    inline std::unordered_map<int, vec_E<common::State>>
    openloop_pred_trajs() const {
    ...
    inline void
    set_obstacle_map(const common::GridMapND<ObstacleMapType, 2>& in) {
        obstacle_map_ = in;
    }
    inline void
    set_obstacle_grids(const std::set<std::array<decimal_t, 2>>& in) {
        obstacle_grids_ = in;
    }
    ...
    inline void
    set_uncertain_vehicle_ids(const std::vector<int>& uncertain_vehicle_ids) {
...
#endif // _CORE_SEMANTIC_MAP_MANAGER_INCLUDE_SEMANTIC_MAP_MANAGER_SEMANTIC_MAP_MANAGER_H_