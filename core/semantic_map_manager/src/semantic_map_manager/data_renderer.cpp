...
ErrorType DataRenderer::Render(const double& time_stamp,
    ...
    if (p_semantic_map_manager_->agent_config_info().enable_tracking_noise) {
        ...
        p_semantic_map_manager_->set_uncertain_vehicle_ids(
            uncertain_vehicle_ids_);
    }
...
ErrorType DataRenderer::InjectObservationNoise() {
    ...
    if (cnt_random_ == 10) {
        ...
        for (auto& v : surrounding_vehicles_.vehicles) {
            ...
            Vec2f augmented_position = Vec2f(
                original_position.x() + lateral_position_noise * sin(angle) +
...
ErrorType
DataRenderer::GetObstacleMap(const common::ObstacleSet& obstacle_set) {
    // ~ NOTICE:
    // ~ Origin of OccupancyGrid is at left-bottom corner,
    // ~ when x -> right, y -> up

    decimal_t x =
        ego_state_.vec_position(0) - obstacle_map_info_.h_metric / 2.0;
    decimal_t y =
        ego_state_.vec_position(1) - obstacle_map_info_.w_metric / 2.0;
    ...
    for (const auto& obs : obstacle_set.obs_circle) {
        ...
        auto center_coord =
            p_obstacle_grid_->GetCoordUsingGlobalPosition(center_w);
        cv::Point2i center(center_coord[0], center_coord[1]);
        int radius = obs.second.circle.radius / obstacle_map_info_.resolution;
        cv::circle(grid_mat, center, radius, cv::Scalar(GridMap2D::OCCUPIED),
                   -1);
    }
...
ErrorType
DataRenderer::GetSurroundingVehicles(const common::VehicleSet& vehicle_set) {
    surrounding_vehicles_.vehicles.clear();

    for (const auto& v : vehicle_set.vehicles) {
        if (v.second.id() == ego_id_)
            continue;
        double dx = v.second.state().vec_position(0) -
                    ego_vehicle_.state().vec_position(0);
        double dy = v.second.state().vec_position(1) -
                    ego_vehicle_.state().vec_position(1);
