...
ErrorType SemanticMapManager::SaveMapToLog() {
    ...
    vehicle_set.vehicles.insert(
        std::make_pair(ego_vehicle_.id(), ego_vehicle_));
...
ErrorType SemanticMapManager::MobilRuleBasedBehaviorPrediction(
    ...
    for (const auto& behavior : behaviors) {
        ...
        GetRefLaneForStateByBehavior(vehicle.state(), std::vector<int>(),
                                     behavior, lane_radius, lane_radius, false,
                                     &ref_lane);
...
ErrorType SemanticMapManager::GetLeadingAndFollowingVehiclesFrenetStateOnLane(
    ...
    *has_following_vehicle = false;
    ...
    if (following_vehicle->id() != kInvalidAgentId) {
        if (stf.GetFrenetStateFromState(following_vehicle->state(),
...
ErrorType
SemanticMapManager::CheckCollisionUsingGlobalPosition(const Vec2f& p_w,
                                                      bool* res) const {
    ...
    while (!queue.empty() && expanded_nodes < max_expansion_nodes) {
        ...
        if (it == semantic_lane_set_.semantic_lanes.end()) {
            ...
            if (it->second.l_change_avbl)
                child_ids.push_back(it->second.l_lane_id);
            if (it->second.r_change_avbl)
                child_ids.push_back(it->second.r_lane_id);
        }
        if (child_ids.empty())
            continue;
        for (auto& id : child_ids) {
            if (visited_set.count(id) == 0) {
                ...
                if (it->second.l_change_avbl && id == it->second.l_lane_id) {
                    num_lc_map.insert(
                        std::make_pair(id, num_lc_map.at(cur_id) + 1));
                } else if (it->second.r_change_avbl &&
                           id == it->second.r_lane_id) {
                    num_lc_map.insert(
                        std::make_pair(id, num_lc_map.at(cur_id) + 1));
                } else {
                    num_lc_map.insert(
                        std::make_pair(id, num_lc_map.at(cur_id)));
                }
            }
        }
    }
...
ErrorType SemanticMapManager::GetNearestLaneIdUsingState(
    ...
    *arc_len = std::get<2>(*lanes_in_angle_diff.begin());
    ...
    // *distance = std::get<0>(*lanes_in_dist.begin());
    // *arc_len = std::get<1>(*lanes_in_dist.begin());
    // printf("[GetNearestLaneIdUsingState]angle_diff: %lf, dist: %lf, id:
    // %d\n",
...
ErrorType SemanticMapManager::UpdateKeyVehicles() {
    ...
    // Find key lane ids
    {
        ...
        key_lane_ids.insert(
            std::pair<int, decimal_t>(cur_lane_id, -cur_arc_len));
        ...
        for (const int& id : mid_lane_ids) {
            ...
            while (!id_to_expand.empty()) {
                ...
                for (const auto& succ_id : succ_ids) {
                    key_lane_ids.insert(
                        std::pair<int, decimal_t>(succ_id, len_sum));
                    decimal_t len_tmp =
                        whole_lane_net_.lane_set.at(succ_id).length +
                        len_expand;
                    if (len_tmp < search_radius) {
                        id_to_expand.insert(
                            std::pair<decimal_t, int>(len_tmp, succ_id));
                    }
                }
            }
        }
        ...
        for (const auto& id : mid_lane_ids) {
            ...
            while (!id_to_expand.empty()) {
                ...
                for (const auto& pred_id : pred_ids) {
                    decimal_t len_tmp =
                        -whole_lane_net_.lane_set.at(pred_id).length +
                        len_expand;
                    key_lane_ids.insert(
                        std::pair<int, decimal_t>(pred_id, len_tmp));
                    if (fabs(len_tmp) < search_radius) {
                        id_to_expand.insert(
                            std::pair<decimal_t, int>(len_tmp, pred_id));
                    }
                }
            }
        }
    ...
    decimal_t pl_horizon_length =
        ego_vehicle_.state().velocity * t_comfort + 100;
            ...
            if (it != key_lane_ids.end()) {
                ...
                if (dist >= 0 && fabs(dist) < front_range) {
                    ...
                    semantic_key_vehicles_.semantic_vehicles.insert(
                        std::pair<int, common::SemanticVehicle>(
                            v_id,
                            semantic_surrounding_vehicles_.semantic_vehicles.at(
                                v_id)));
                    key_vehicles_.vehicles.insert(
                        std::pair<int, common::Vehicle>(
                            v_id,
                            semantic_surrounding_vehicles_.semantic_vehicles
                                .at(v_id)
                                .vehicle));
                }
                // * Rear
                decimal_t s_margin = std::max(
                    20.0, fabs(v.second.vehicle.state().velocity) * t_pl);
...
ErrorType SemanticMapManager::GetLocalLaneSamplesByState(
    ...
    common::Lane target_lane =
        semantic_lane_set_.semantic_lanes.at(lane_id).lane;
    ...
    decimal_t forward_sample_len =
        std::min(max_reflane_dist, accum_dist_forward);
...
ErrorType
SemanticMapManager::GetLaneBySampledPoints(const vec_Vecf<2>& samples,
    ...
    for (decimal_t s = ref_fs.vec_s[0] + resolution + search_lon_offset;
        ...
        for (const auto& entry : vehicle_set.vehicles) {
            ...
            if ((lane_pt - entry.second.state().vec_position).squaredNorm() <
                ...
                *distance_residual_ratio = (max_forward_search_dist - delta_s) /
                                           max_forward_search_dist;
                break;
            }
        }
...
ErrorType SemanticMapManager::GetFollowingVehicleOnLane(
    ...
    common::FrenetState ref_fs;
    if (stf.GetFrenetStateFromState(ref_state, &ref_fs) != kSuccess) {
        // printf("[FollowingVehicleOnLane]Cannot get ref state frenet
        // state.\n");
        return kWrongStatus;
    }
