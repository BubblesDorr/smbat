...
ErrorType TrafficSignalManager::GetSpeedLimit(const State& state,
    ...
    for (auto& speed_limit : speed_limit_list_) {
        ...
        if (CheckIntersectionTypeWithSignal(
                ref_fs, lane, speed_limit, &intersection_type, &dist_to_startpt,
        ...
        if (intersection_type != kNotIntersect) {
            decimal_t effect_speed_limit_dist =
                state.velocity > speed_limit.max_velocity()
                    ? fabs(speed_limit.max_velocity() *
                               speed_limit.max_velocity() -
                           state.velocity * state.velocity) /
                          (2.0 * acc_esti)
                    : 0.0;
            // printf("[denny]cur vel %lf limit max vel %lf, effect dist
            // %lf.\n",
            //        state.velocity, speed_limit.max_velocity(),
            ...
            if ((intersection_type == kSignalAhead &&
                ...
                limit = limit > speed_limit.max_velocity()
                            ? speed_limit.max_velocity()
                            : limit;
            }
        }
...
ErrorType TrafficSignalManager::CheckIntersectionTypeWithSignal(
    ...
    if (stf.GetFrenetPointFromPoint(signal.end_point(), &end_pt_fs) !=
        kSuccess) {
        return kWrongStatus;
    }
    ...
    // std::cout << "[denny]start pt fs" << start_pt_fs.transpose() <<
    // std::endl; std::cout << "[denny]end pt fs" << end_pt_fs.transpose() <<
    // std::endl;
    if (start_pt_fs[0] < end_pt_fs[0] + kEPS) {
        if (start_pt_fs[1] + lateral_range(1) > 0.0 &&
            ...
            } else if (fs.vec_s[0] >= start_pt_fs[0] &&
                       fs.vec_s[0] < end_pt_fs[0]) {
