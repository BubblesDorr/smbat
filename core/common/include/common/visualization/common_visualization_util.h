...
class VisualizationUtil {
  public:
    ...
    static ErrorType GetMarkerByPolynomial(
        const PolynomialND<N_DEG, N_DIM>& poly, const decimal_t s0,
    ...
    static ErrorType GetMarkerByLane(const Lane& lane, const decimal_t step,
        ...
        GetMarkerBySpline<LaneDegree, LaneDim>(lane.position_spline(), step,
                                               scale, color, offset_z, marker);
        return kSuccess;
    }
     ...
     */
    static ErrorType
    GetMarkerArrayByTrajectory(const Trajectory& traj, const decimal_t step,
                               const Vec3f& scale, const ColorARGB& color,
                               const decimal_t offset_z,
     ...
     */
    static ErrorType
    GetMarkerArrayByStateVector(const vec_E<State>& state_vec,
                                const ColorARGB& color,
                                visualization_msgs::MarkerArray* marker_arr) {
        visualization_msgs::Marker traj_mk;
        traj_mk.type = visualization_msgs::Marker::LINE_STRIP;
     ...
     */
    static ErrorType
    FillScaleColorInMarker(const Vec3f scale, const ColorARGB color,
                           visualization_msgs::Marker* marker) {
     ...
     */
    static ErrorType
    FillGradientColorInMarker(const bool if_ascending,
                              visualization_msgs::Marker* marker) {
     ...
     */
    static ErrorType
    FillHeaderIdInMarkerArray(const ros::Time time_stamp,
     ...
     */
    static ErrorType
    FillStampInMarkerArray(const ros::Time time_stamp,
                           visualization_msgs::MarkerArray* marker_arr) {
        for (auto& mk : marker_arr->markers) {
            mk.header.stamp = time_stamp;
        }
     ...
     */
    static ErrorType
    FillLifeTimeInMarkerArray(const ros::Duration duration,
                              visualization_msgs::MarkerArray* marker_arr) {
        for (auto& mk : marker_arr->markers) {
     ...
     */
    static ErrorType
    GetRosPoseStampedFromVec3d(const Vec3f& state,
                               geometry_msgs::PoseStamped* p_pose_stamped) {
     ...
     */
    static ErrorType
    GetRosPoseArrayFromState3dVector(const std::vector<Vec3f>& states,
     ...
     */
    static ErrorType
    GetRosPointCloudFrom3DofStateVector(const std::vector<Vec3f>& states,
     ...
     */
    static ErrorType
    GetRosPointCloudFromCircleArc(const CircleArc& arc,
                                  sensor_msgs::PointCloud* p_pc) {
        p_pc->header.frame_id = "map";
        p_pc->header.stamp = ros::Time::now();
     ...
     */
    static ErrorType
    GetRosPointCloudFromCircleArcBranch(const CircleArcBranch& arc_branch,
                                        sensor_msgs::PointCloud* p_pc) {
        p_pc->header.frame_id = "map";
        p_pc->header.stamp = ros::Time::now();
     ...
     */
    static ErrorType
    GetRosPointCloudFromPointList(const std::vector<common::Point>& point_list,
                                  sensor_msgs::PointCloud* p_pc) {
        p_pc->header.frame_id = "map";
        p_pc->header.stamp = ros::Time::now();
     ...
     */
    static ErrorType
    GetRosMarkerSphereUsingPoint(const Vec3f& pt, const ColorARGB& color,
     ...
     */
    static ErrorType
    GetRosMarkerCylinderUsingCircle(const Circle& circle,
     ...
     */
    static ErrorType
    GetRosMarkerCylinderUsingPoint(const Point& pt, const Vec3f& scale,
                                   const ColorARGB& color, const int& id,
                                   visualization_msgs::Marker* p_marker) {
        p_marker->type = visualization_msgs::Marker::CYLINDER;
        p_marker->action = visualization_msgs::Marker::MODIFY;
    ...
    /**
     * @brief Get the Ros Marker Cube Using Oriented Bounding Box 2 D With
     * Offset Z object
     *
     * @param obb
     * @param offset_z
     ...
     */
    static ErrorType
    GetRosMarkerMeshConeUsingPosition(const Vec3f& pos, const ColorARGB& color,
                                      const int& id,
                                      visualization_msgs::Marker* p_marker) {
    ...
    static ErrorType
    GetRosMarkerArrowUsingOriginAndVector(const Vec3f& origin, const Vec3f& vec,
    ...
    static ErrorType GetRosMarkerLineStripUsing2DofVecWithOffsetZ(
        const vec_E<Vec2f>& path, const ColorARGB& color, const Vec3f& scale,
        const decimal_t& z, const int& id,
        visualization_msgs::Marker* p_marker) {
        p_marker->type = visualization_msgs::Marker::LINE_STRIP;
        p_marker->action = visualization_msgs::Marker::MODIFY;
        p_marker->id = id;
     ...
     */
    static ErrorType
    GetRosMarkerLineStripUsingPoints(const std::vector<Point> points,
                                     const Vec3f& scale, const ColorARGB& color,
                                     const int& id,
                                     visualization_msgs::Marker* p_marker) {
        p_marker->type = visualization_msgs::Marker::LINE_STRIP;
    ...
    static ErrorType GetRosMarkerArrayUsingVehicle(
        ...
        GetRosMarkerMeshUsingOrientedBoundingBox2D(obb, color_obb,
                                                   &mesh_marker);
        ...
        horizontal_line.emplace_back(
            state3df[0] + line_width * sin(state3df[2]),
            state3df[1] - line_width * cos(state3df[2]), -0.2);
        horizontal_line.emplace_back(
            state3df[0] - line_width * sin(state3df[2]),
            state3df[1] + line_width * cos(state3df[2]), -0.2);
        GetRosMarkerLineStripUsing3DofStateVec(horizontal_line, -0.4,
                                               color_steer, Vec3f(0.1, 0, 0),
                                               id + 6, &horizontal_marker);
     ...
     */
    static ErrorType
    GetRosMarkerUsingCircleObstacle(const CircleObstacle& obs,
                                    visualization_msgs::Marker* p_marker) {
        p_marker->header.frame_id = "map";
        p_marker->header.stamp = ros::Time::now();
        GetRosMarkerCylinderUsingCircle(
            obs.circle, ColorARGB(0.5, 1.0, 1.0, 1.0), obs.id, p_marker);
        return kSuccess;
    }
     ...
     */
    static ErrorType
    GetRosMarkerUsingPolygonObstacle(const PolygonObstacle& obs, const int& id,
                                     visualization_msgs::Marker* p_marker) {
        ...
        p_marker->header.frame_id = "map";
        p_marker->header.stamp = ros::Time::now();
        GetRosMarkerLineStripUsingPoints(points, Vec3f(0.2, 0, 0),
                                         ColorARGB(1.0, 0.7, 0.7, 0.7), id,
                                         p_marker);
        return kSuccess;
    }
    ...
    static ErrorType GetRosMarkerUsingObstacleSet(
        ...
        for (const auto& p_obs : obstacles.obs_polygon) {
            ...
            case 0: {
                visualization_msgs::Marker obs_marker;
                GetRosMarkerUsingPolygonObstacle(p_obs.second, id_cnt,
                                                 &obs_marker);
                p_marker_array->markers.push_back(obs_marker);
                ++id_cnt;
                break;
            ...
            case 1: {
                for (const auto& pt : p_obs.second.polygon.points) {
                    ...
                    GetRosMarkerMeshConeUsingPosition(pos, cmap.at("yellow"),
                                                      id_cnt, &obs_marker);
                    p_marker_array->markers.push_back(obs_marker);
                    ++id_cnt;
                }
    ...
    static ErrorType GetRosMarkerArrUsingSemanticBehavior(
        ...
        // lane direction marker
        {
            if (behavior.ref_lane.IsValid()) {
                ...
                decimal_t angle =
                    0.0; // angle between horizontal line & direction
                if (behavior.lat_behavior ==
                    ...
                    FillScaleColorInMarker(Vec3f(0.4, 0.0, 0.0),
                                           ColorARGB(1.0, 1.0, 1.0, 0.0),
                                           &direction_mk);
                    angle = kPi / 4.0;
                } else if (behavior.lon_behavior ==
                           common::LongitudinalBehavior::kDecelerate) {
                    FillScaleColorInMarker(Vec3f(0.4, 0.0, 0.0),
                                           ColorARGB(1.0, 1.0, 0.0, 0.0),
                                           &direction_mk);
                    angle = 0.0;
                } else {
                    FillScaleColorInMarker(Vec3f(0.4, 0.0, 0.0),
                                           ColorARGB(1.0, 0.1, 0.8, 0.1),
                                           &direction_mk);
                    angle = kPi / 4.0;
                }
                ...
                for (decimal_t s = behavior.ref_lane.begin();
                    ...
                    Vecf<2> normal_vec;
                    behavior.ref_lane.GetNormalVectorByArcLength(s,
                                                                 &normal_vec);
                ...
                for (const auto& p_traj : surround_trajs) {
                    ...
                    for (const auto& v : p_traj.second) {
                        ...
                        common::VisualizationUtil::
                            GetRosMarkerCylinderUsingPoint(
                                common::Point(pt), Vec3f(0.5, 0.5, 0.1),
                                traj_color, ++cnt, &point_marker);
                    ...
                    common::VisualizationUtil::GetRosMarkerLineStripUsingPoints(
                        points, Vec3f(0.1, 0.1, 0.1), traj_color, ++cnt,
                        &line_marker);
    ...
    template <typename T>
    static ErrorType
    GetRosOccupancyGridUsingGripMap2D(const GridMapND<T, 2>& grid_map,
...
#endif // _CORE_COMMON_INCLUDE_COMMON_VISUALIZATION_COMMON_VISUALIZATION_UTIL_H_