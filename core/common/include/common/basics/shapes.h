#ifndef _CORE_COMMON_INCLUDE_COMMON_BASICS_SHAPES_H_
#define _CORE_COMMON_INCLUDE_COMMON_BASICS_SHAPES_H_
...
template <typename T> struct PointWithValue {
    Point pt;
    std::vector<T> values;
};
...
template <int N> struct AxisAlignedBoundingBoxND {
...
template <typename T, int N_DIM> struct AxisAlignedCubeNd {
...
class ShapeUtils {
  public:
     ...
     */
    static bool
    CheckIfOrientedBoundingBoxIntersect(const OrientedBoundingBox2D& obb_a,
     ...
     */
    static ErrorType
    GetVerticesOfOrientedBoundingBox(const OrientedBoundingBox2D& obb,
                                     vec_E<Vecf<2>>* vertices);
     ...
     */
    static ErrorType
    GetPerpendicularAxesOfOrientedBoundingBox(const vec_E<Vecf<2>>& vertices,
                                              vec_E<Vecf<2>>* axes);
     ...
     */
    static ErrorType
    GetPerpendicularAxisOfOrientedBoundingBox(const vec_E<Vecf<2>>& vertices,
                                              const int index, Vecf<2>* axis);
     ...
     */
    static ErrorType
    GetCvPoint2iVecUsingCommonPoint2iVec(const std::vector<Point2i>& pts_in,
    ...
    static bool CheckIfAxisAlignedCubeCollide(
        ...
        for (int i = 0; i < N_DIM; ++i) {
            decimal_t half_len_a =
                fabs(static_cast<double>(aabb_a.upper_bound[i] -
                                         aabb_a.lower_bound[i]) /
                     2.0);
            decimal_t half_len_b =
                fabs(static_cast<double>(aabb_b.upper_bound[i] -
                                         aabb_b.lower_bound[i]) /
                     2.0);

            decimal_t center_a = static_cast<double>(aabb_a.upper_bound[i] +
                                                     aabb_a.lower_bound[i]) /
                                 2.0;
            decimal_t center_b = static_cast<double>(aabb_b.upper_bound[i] +
                                                     aabb_b.lower_bound[i]) /
                                 2.0;
...
#endif // _CORE_COMMON_INCLUDE_COMMON_BASICS_SHAPES_H_