#ifndef _CORE_COMMON_INCLUDE_COMMON_STATE_FRENET_STATE_H_
#define _CORE_COMMON_INCLUDE_COMMON_STATE_FRENET_STATE_H_
    ...
    void Load(const Vecf<3>& s, const Vecf<3>& d, const InitType& type) {
        ...
        if (type == kInitWithDt) {
            ...
            if (fabs(vec_s[1]) > kEPS) {
                ...
                vec_ds[2] =
                    (vec_dt[2] - vec_ds[1] * vec_s[2]) / (vec_s[1] * vec_s[1]);
                is_ds_usable = true;
            } else {
                vec_ds[1] = 0.0;
                vec_ds[2] = 0.0;
...
#endif // _CORE_COMMON_INCLUDE_COMMON_STATE_FRENET_STATE_H_