#ifndef _CORE_COMMON_INCLUDE_COMMON_SPLINE_POLYNOMIAL_H_
#define _CORE_COMMON_INCLUDE_COMMON_SPLINE_POLYNOMIAL_H_
...
template <int N_DEG> class Polynomial {
  public:
    ...
    inline decimal_t evaluate(const decimal_t& s, const int& d) const {
        ...
        return p;
        // After testing, above is generally faster than below (obvious when d =
        // 1, less obvious as d increases) decimal_t p =
        // coeff_normal_order_[N_DEG] * fac(N_DEG) / fac(N_DEG - d); for (int i
        // = 1; i <= N_DEG - d; i++) {
    ...
    inline decimal_t J(decimal_t s, int d) const {
        if (d == 3) {
            ...
            return coeff_(0) * coeff_(0) / 20.0 * pow(s, 5) +
                   ...
                   (coeff_(1) * coeff_(1) + coeff_(0) * coeff_(2)) / 3 *
                       pow(s, 3) +
...
#endif // _CORE_COMMON_INCLUDE_COMMON_SPLINE_POLYNOMIAL_H_