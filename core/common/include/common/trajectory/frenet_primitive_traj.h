#ifndef _CORE_COMMON_INCLUDE_COMMON_TRAJECTORY_FRENET_PRIMITIVE_TRAJ_H_
#define _CORE_COMMON_INCLUDE_COMMON_TRAJECTORY_FRENET_PRIMITIVE_TRAJ_H_
...
class FrenetPrimitiveTrajectory : public FrenetTrajectory {
  public:
    ...
    ErrorType GetFrenetState(const decimal_t& t,
                             FrenetState* fs) const override {
        if (t < begin() - kEPS || t > end() + kEPS)
            return kWrongStatus;
        if (primitive_.GetFrenetState(t, fs) != kSuccess) {
            return kWrongStatus;
...
#endif // _CORE_COMMON_INCLUDE_COMMON_TRAJECTORY_FRENET_PRIMITIVE_TRAJ_H_