...
ErrorType IntelligentDriverModel::GetIIdmDesiredAcceleration(
    ...
    decimal_t a_free =
        cur_state.v <= param.kDesiredVelocity
            ? param.kAcceleration *
                  (1 -
                   pow(cur_state.v / param.kDesiredVelocity, param.kExponent))
            : -param.kComfortableBrakingDeceleration *
    ...
    decimal_t z =
        (param.kMinimumSpacing +
         std::max(0.0, cur_state.v * param.kDesiredHeadwayTime +
                           cur_state.v * (cur_state.v - cur_state.v_front) /
                               (2.0 *
                                sqrt(param.kAcceleration *
                                     param.kComfortableBrakingDeceleration)))) /
        s_alpha;
    decimal_t a_out =
        cur_state.v <= param.kDesiredVelocity
...
ErrorType IntelligentDriverModel::GetAccDesiredAcceleration(
    ...
    if (acc_iidm >= acc_cah) {
        ...
        *acc = (1 - coolness) * acc_iidm +
               coolness *
                   (acc_cah - param.kComfortableBrakingDeceleration *
