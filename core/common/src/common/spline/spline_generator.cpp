...
ErrorType SplineGenerator<N_DEG, N_DIM>::GetCubicSplineBySampleInterpolation(
    ...
    if (samples.size() != para.size())
        return kIllegalInput;
    if (N_DEG < 3) {
        printf("[CubicSplineBySampleInterpolation]Currently we just support "
            ...
            for (int n = 0; n < n_inserted; n++) {
                ...
                if (para_insert < para[i]) {
                    ...
                    for (int d = 0; d < N_DIM; d++) {
                        sample_insert[d] =
                            (samples[i][d] - samples[i - 1][d]) * partion +
                            samples[i - 1][d];
                    }
    ...
    for (int i = 0; i < N_DIM; i++) {
        ...
        for (int s = 0; s < static_cast<int>(interpolated_samples.size());
             s++) {
...
ErrorType SplineGenerator<N_DEG, N_DIM>::GetQuinticSplineBySampleFitting(
        ...
        for (int n = 0; n < num_segments; n++) {
            ...
            for (int i = 0; i < num_sub_samples; i++) {
                ...
                for (int j = 0; j < num_order; j++) {
                    val =
                        pow(deltat, num_order - j - 1) / fac(num_order - j - 1);
                    for (int d = 0; d < N_DIM; d++) {
    ...
    C.reserve(Eigen::VectorXi::Constant(
        N_DIM * num_connections * num_continuity, 2 * num_order));
    {
        int idx, idy_l, idy_r;
        double val_l, val_r;
        for (int n = 0; n < num_connections; n++) {
            for (int c = 0; c < num_continuity; c++) {
                for (int j = 0; j < num_order; j++) {
                    if (j <= num_order - 1 - c) {
                        auto T = durations[n];
                        val_l = pow(T, num_order - j - 1 - c) /
                                fac(num_order - j - 1 - c);
                        val_r = -pow(0.0, num_order - j - 1 - c) /
                                fac(num_order - j - 1 - c);
                        for (int d = 0; d < N_DIM; d++) {
                            idx = d * num_connections * num_continuity +
                                  n * num_continuity + c;

                            idy_l = d * num_segments * num_order +
                                    n * num_order + j;
                            idy_r = d * num_segments * num_order +
                                    (n + 1) * num_order + j;
        ...
        for (int d = 0; d < N_DIM; d++) {
            for (int n = 0; n < num_segments; n++) {
                ...
                for (int i = 0; i <= N_DEG; i++) {
                    coeff(i) =
                        x[d * num_segments * num_order + n * num_order + i];
                }
...
ErrorType SplineGenerator<N_DEG, N_DIM>::GetSplineFromStateVec(
    ...
    SplineType* spline) {
    if (N_DEG < 5) {
        printf("[GetSplineFromStateVec]To fit state vec we need at least 5 "
               "degree.\n");
        return kWrongStatus;
    }
    if (para.size() < 2) {
...
ErrorType SplineGenerator<N_DEG, N_DIM>::GetBezierSplineUsingCorridor(
            ...
            for (int d = 0; d < N_DIM; d++) {
                for (int j = 0; j < num_order; j++) {
                    for (int k = 0; k < num_order; k++) {
                        ...
                        val = hessian(j, k) /
                              pow(duration, 2 * derivative_degree - 3);
            ...
            for (int d = 0; d < N_DIM; d++) {
                for (int j = 0; j < num_order; j++) {
                    ...
                    for (int k = 0; k < num_order; k++) {
                        idy = d * num_segments * num_order + n * num_order + k;
                        P.coeffRef(idx, idy) +=
                            s * s * nchoosek(N_DEG, j) * nchoosek(N_DEG, k) *
                            pow(t / s, j + k) *
    ...
    A.reserve(
        Eigen::VectorXi::Constant(total_num_eq_constraints, 2 * num_order));
    ...
    decimal_t val;
    {
        ...
        for (int n = 0; n < num_connections; n++) {
            ...
            for (int c = 0; c < num_continuity; c++) {
                ...
                for (int d = 0; d < N_DIM; d++) {
                    ...
                    if (c == 0) {
                        // ~ position end
                        idy = d * num_segments * num_order + n * num_order +
                              N_DEG;
                        val = 1.0 * scale_l;
                        A.insert(idx, idy) = val;
                        // ~ position begin
                        idy = d * num_segments * num_order +
                              (n + 1) * num_order + 0;
                        val = 1.0 * scale_r;
                        A.insert(idx, idy) = -val;
                    } else if (c == 1) {
                        ...
                        // ~ velocity begin
                        idy =
                            d * num_segments * num_order + (n + 1) * num_order;
                        val = -1.0 * scale_r;
                        A.insert(idx, idy) = -val;
                        idy = d * num_segments * num_order +
                              (n + 1) * num_order + 1;
        ...
        // ~ start state constraints
        {
            ...
            for (int j = 0; j < num_order_constraint_start; j++) {
                ...
                for (int d = 0; d < N_DIM; d++) {
                    idx = num_continuity_constraints +
                          d * num_order_constraint_start + j;
                    if (j == 0) {
        ...
        {
            int num_order_constraint_end =
                static_cast<int>(end_constraints.size());
            decimal_t duration =
        ...
        for (int n = 0; n < num_segments; n++) {
            ...
            for (int d = 0; d < N_DIM; d++) {
                ...
                for (int j = 0; j < num_order - 1; j++) {
                    ...
                    C.insert(idx, idy) = val;
                    idy =
                        d * num_segments * num_order + n * num_order + (j + 1);
                    val = N_DEG * scale;
                    C.insert(idx, idy) = val;
                ...
                for (int j = 0; j < num_order - 2; j++) {
                    ...
                    C.insert(idx, idy) = val;
                    idy =
                        d * num_segments * num_order + n * num_order + (j + 2);
                    val = N_DEG * (N_DEG - 1) * scale;
                    C.insert(idx, idy) = val;
