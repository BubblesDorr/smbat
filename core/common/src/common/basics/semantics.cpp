...
template <typename T, int N_DIM> GridMapND<T, N_DIM>::GridMapND() {}
...
template <typename T, int N_DIM>
std::array<int, N_DIM>
GridMapND<T, N_DIM>::GetNDimIdxUsingMonoIdx(const int& idx) const {
...
template <typename T, int N_DIM>
ErrorType
GridMapND<T, N_DIM>::SetNDimSteps(const std::array<int, N_DIM>& dims_size) {
    int step = 1;
    for (int i = 0; i < N_DIM; ++i) {
        dims_step_[i] = step;
...
template <typename T, int N_DIM>
ErrorType
GridMapND<T, N_DIM>::SetDataSize(const std::array<int, N_DIM>& dims_size) {
    int total_ele_num = 1;
    for (int i = 0; i < N_DIM; ++i) {
...
TrafficSignal::TrafficSignal()
    : start_point_(Vec2f::Zero()), end_point_(Vec2f::Zero()),
      valid_time_(Vec2f(0.0, std::numeric_limits<decimal_t>::max())),
      vel_range_(Vec2f::Zero()), lateral_range_(Vec2f(-1.75, 1.75)) {}

TrafficSignal::TrafficSignal(const Vec2f& start_point, const Vec2f& end_point,
                             const Vec2f& valid_time, const Vec2f& vel_range)
    : start_point_(start_point), end_point_(end_point), valid_time_(valid_time),
      vel_range_(vel_range), lateral_range_(Vec2f(-1.75, 1.75)) {}
