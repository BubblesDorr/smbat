...
ErrorType StateTransformer::GetStateFromFrenetState(const FrenetState& fs,
    ...
    decimal_t delta_theta_derivative =
        1.0 / (1 + tan_delta_theta * tan_delta_theta) *
...
ErrorType StateTransformer::GetFrenetStateFromState(const State& s,
    ...
    Vecf<2> lane_normal_vec =
        Vecf<2>(-lane_tangent_vec[1], lane_tangent_vec[0]);
    ...
    decimal_t delta_theta_derivative =
        ...
        (dss * one_minus_curd + curvature * pow(ds, 2)) /
        pow(one_minus_curd, 2);
...
ErrorType
StateTransformer::GetFrenetPointVectorFromPoints(const vec_E<Vec2f>& pts,
                                                 vec_E<Vec2f>* fps) const {
    fps->clear();
