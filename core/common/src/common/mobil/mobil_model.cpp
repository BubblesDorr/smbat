...
ErrorType MobilLaneChangingModel::GetMobilAccChangesOnCurrentLane(
    ...
    if ((!has_following_vehicle) || fabs(following_fs.vec_s(1)) < kEPS) {
        ...
        if (!has_leading_vehicle) {
            GetDesiredAccelerationUsingIdm(
                idm_param_c, cur_fs, common::FrenetState(), true, &acc_c_tmp);
        } else {
            GetDesiredAccelerationUsingIdm(idm_param_c, cur_fs, leading_fs,
                                           false, &acc_c_tmp);
        }
    } else {
        if (!has_leading_vehicle) {
            // ~ Without leading vehicle, use virtual vehicle
            GetDesiredAccelerationUsingIdm(idm_param_o, following_fs, cur_fs,
                                           false, &acc_o_tmp);
            ...
            GetDesiredAccelerationUsingIdm(idm_param_o, following_fs, cur_fs,
                                           false, &acc_o_tmp);
            GetDesiredAccelerationUsingIdm(idm_param_o, following_fs,
                                           leading_fs, false, &acc_o_tilda_tmp);
            GetDesiredAccelerationUsingIdm(idm_param_c, cur_fs, leading_fs,
                                           false, &acc_c_tmp);
        }
    }
...
ErrorType MobilLaneChangingModel::GetDesiredAccelerationUsingIdm(
    ...
    if (!use_virtual_front) {
        ...
        idm_state = IntelligentDriverModel::State(0.0, rear_fs.vec_s(1),
                                                  100.0 + rear_fs.vec_s(1) * 10,
                                                  rear_fs.vec_s(1));
    }
