...
ErrorType MobilBehaviorPrediction::LateralBehaviorPrediction(
    ...
    if (lanes.size() != 3) {
        printf("[MobilBehaviorPrediction]Must have three lanes (invalid also "
               "acceptable).\n");
        return kWrongStatus;
    }
    ...
    if (lk_lane.IsValid()) {
        ...
        if (MobilLaneChangingModel::GetMobilAccChangesOnCurrentLane(
                ego_frenet_state, leading_vehicle, leading_fs,
                following_vehicle, following_fs, &acc_o, &acc_o_tilda,
                &acc_c) != kSuccess) {
    ...
    if (lcl_lane.IsValid()) {
        ...
        MobilLaneChangingModel::GetMobilAccChangesOnTargetLane(
            projected_frenet_state, leading_vehicle, leading_fs,
            following_vehicle, following_fs, &is_lcl_safe, &acc_n, &acc_n_tilda,
            &acc_c_tilda);
        if (is_lcl_safe) {
    ...
    if (lcr_lane.IsValid()) {
        ...
        MobilLaneChangingModel::GetMobilAccChangesOnTargetLane(
            projected_frenet_state, leading_vehicle, leading_fs,
            following_vehicle, following_fs, &is_lcr_safe, &acc_n, &acc_n_tilda,
            &acc_c_tilda);
        if (is_lcr_safe) {
            decimal_t d_a_c = acc_c_tilda - acc_c;
            decimal_t d_a_n = acc_n_tilda - acc_n;
    ...
    RemapGainsToProb(is_lcl_safe, mobil_gain_left, is_lcr_safe,
                     mobil_gain_right, res);
