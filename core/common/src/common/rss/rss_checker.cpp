    ...
    if (direction == Front) {
        ...
        if (ego_vel >= 0.0 && other_vel >= 0.0) {
            ...
            other_distance_driven = (other_vel_abs * other_vel_abs) /
                                    (2 * config.longitudinal_brake_max);
            ret = ego_distance_driven - other_distance_driven;
        } else if (ego_vel >= 0.0 && other_vel <= 0.0) {
            // ego vehicle ==> <-- other vehicle
            other_distance_driven =
                (other_vel_abs + other_vel_at_response_time) / 2.0 *
                    config.response_time +
                other_vel_at_response_time * other_vel_at_response_time /
                    (2 * config.longitudinal_brake_min);
            ret = ego_distance_driven + other_distance_driven;
        } else {
...
ErrorType RssChecker::RssCheck(const Vehicle& ego_vehicle,
    ...
    CalculateSafeLateralDistance(ego_fs.vec_dt[1], other_fs.vec_dt[1],
                                 lat_direct, config, &safe_lat_distance);
    safe_lat_distance +=
    ...
    if (lon_direct == Rear) {
        ...
        decimal_t ego_rear_wheel_to_back_bump = fabs(
        ...
        decimal_t ego_rear_wheel_to_front_bump =
            0.5 * ego_vehicle.param().length() + ego_vehicle.param().d_cr();
        decimal_t other_rear_wheel_to_back_bump =
            fabs(0.5 * other_vehicle.param().length() -
                 other_vehicle.param().d_cr());
        lon_distance_abs = fabs(ego_fs.vec_s[0] - other_fs.vec_s[0]) -
