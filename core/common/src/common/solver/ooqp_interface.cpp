...
bool OoQpItf::solve(const Eigen::SparseMatrix<double, Eigen::RowMajor>& Q,
    ...
    if (verbose) {
        printProblemFormulation(Q_triangular, ccopy, Acopy, bcopy, Ccopy, d, f,
                                l, u);
    }

    // Compress sparse Eigen matrices (refer to Eigen Sparse Matrix user
    // manual).
...
void OoQpItf::printProblemFormulation(
    ...
    cout << "-------------------------------" << endl;
    cout
        << "Find x: min 1/2 x' Q x + c' x such that A x = b, d <= Cx <= f, and "
           "l <= x <= u"
        << endl
        << endl;
