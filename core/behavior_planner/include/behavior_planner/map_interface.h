#ifndef _CORE_BEHAVIOR_PLANNER_INCLUDE_BEHAVIOR_PLANNER_MAP_INTERFACE_H_
#define _CORE_BEHAVIOR_PLANNER_INCLUDE_BEHAVIOR_PLANNER_MAP_INTERFACE_H_
...
class BehaviorPlannerMapItf {
  public:
    ...
    virtual ErrorType GetEgoLaneIdByPosition(const std::vector<int>& navi_path,
                                             int* lane_id) = 0;
    virtual ErrorType
    GetNearestLaneIdUsingState(const Vec3f& state,
                               const std::vector<int>& navi_path, int* id,
    ...
    virtual ErrorType
    GetKeySemanticVehicles(common::SemanticVehicleSet* key_vehicle_set) = 0;
    ...
    virtual ErrorType GetLocalLaneSamplesByState(
        const State& state, const int lane_id,
        const std::vector<int>& navi_path, const decimal_t max_reflane_dist,
        const decimal_t max_backward_dist, vec_Vecf<2>* samples) = 0;
    ...
    virtual ErrorType
    CheckCollisionUsingState(const common::VehicleParam& param_a,
                             const common::State& state_a,
                             const common::VehicleParam& param_b,
                             const common::State& state_b, bool* res) = 0;

    virtual ErrorType
    CheckIfCollision(const common::VehicleParam& vehicle_param,
    ...
    virtual ErrorType GetLeadingVehicleOnLane(
        ...
        decimal_t* distance_residual_ratio) = 0;

    virtual ErrorType GetSpeedLimit(const State& state, const Lane& lane,
                                    decimal_t* speed_limit) = 0;

    virtual ErrorType
    GetPredictedBehavior(const int vehicle_id,
                         common::LateralBehavior* lat_behavior) = 0;
};

} // namespace planning

#endif // _CORE_BEHAVIOR_PLANNER_INCLUDE_BEHAVIOR_PLANNER_MAP_INTERFACE_H_