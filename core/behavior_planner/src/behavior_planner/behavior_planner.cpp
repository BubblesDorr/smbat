...
ErrorType BehaviorPlanner::RunOnce() {
    ...
    if (autonomous_level_ >= 3) {
        TicToc timer;
        planning::MultiModalForward::ParamLookUp(aggressive_level_,
                                                 &sim_param_);
        if (RunMpdm() != kSuccess) {
            printf("[Summary]Mpdm failed: %lf ms.\n", timer.toc());
            // printf("[Stuck]Ego id %d on lane %d with behavior %d mpdm
            // failed.\n",
...
ErrorType
BehaviorPlanner::MultiBehaviorJudge(const decimal_t previous_desired_vel,
                                    LateralBehavior* mpdm_behavior,
                                    decimal_t* mpdm_desired_velocity) {
    ...
    if (lock_to_hmi_) {
        auto it = std::find(forward_behaviors_.begin(),
                            forward_behaviors_.end(), hmi_behavior_);
...
ErrorType BehaviorPlanner::OpenloopSimForward(
    ...
    for (int i = 0; i < num_steps_forward; i++) {
        ...
        for (auto& v : semantic_vehicle_set_tmp.semantic_vehicles) {
            decimal_t desired_vel = agent_vehicles.semantic_vehicles.at(v.first)
                                        .vehicle.state()
                                        .velocity;
        ...
        map_itf_->CheckIfCollision(ego_semantic_vehicle.vehicle.param(),
                                   ego_state, &is_collision);
        if (is_collision)
            return kWrongStatus;
        ...
        for (auto& s : state_cache) {
            semantic_vehicle_set_tmp.semantic_vehicles.at(s.first)
                .vehicle.set_state(s.second);
...
ErrorType
BehaviorPlanner::EvaluateSafetyCost(const vec_E<common::Vehicle>& traj_a,
    ...
    for (int i = 0; i < num_states; i++) {
        ...
        if (is_collision) {
            cost_tmp +=
                0.01 *
                fabs(traj_a[i].state().velocity - traj_b[i].state().velocity) *
                0.5;
        }
    }
    *cost = cost_tmp;
...
ErrorType BehaviorPlanner::EvaluateSinglePolicyTraj(
    ...
    for (auto it = surround_traj.begin(); it != surround_traj.end(); ++it) {
        if (!it->second.empty()) {
            vehicle_set.vehicles.insert(
                std::make_pair(it->first, it->second.back()));
        }
    }
    // * efficiency
    ...
    if (map_itf_->GetRefLaneForStateByBehavior(
        ...
        printf(
            "fail to get ego ref lane duration evaluation for behavior %d.\n",
    ...
    if (map_itf_->GetLeadingVehicleOnLane(
        ...
        if (ego_vehicle_terminal.state().velocity <
                reference_desired_velocity_ &&
...
ErrorType BehaviorPlanner::MultiAgentSimForward(
    ...
    for (int i = 0; i < num_steps_forward; i++) {
        ...
        for (auto& v : semantic_vehicle_set_tmp.semantic_vehicles) {
            decimal_t desired_vel =
                semantic_vehicle_set.semantic_vehicles.at(v.first)
                    .vehicle.state()
                    .velocity;
            decimal_t init_stamp =
                semantic_vehicle_set.semantic_vehicles.at(v.first)
                    .vehicle.state()
                    .time_stamp;
            if (v.first == ego_id)
                desired_vel = reference_desired_velocity_;
            ...
            const decimal_t lat_range = 2.2;
            if (map_itf_->GetLeadingVehicleOnLane(
                    v.second.lane, v.second.vehicle.state(), vehicle_set,
                    lat_range, &leading_vehicle,
...
ErrorType
BehaviorPlanner::ConstructReferenceLane(const LateralBehavior& lat_behavior,
                                        Lane* lane) {
    if (map_itf_ == nullptr)
        return kWrongStatus;
    ...
    if (lat_behavior == common::LateralBehavior::kLaneKeeping ||
        ...
        if (map_itf_->GetLeftLaneId(ego_lane_id_, &target_lane_id) !=
            kSuccess) {
            printf(
                "[BP]Commanding a left lane change, but no existing lane.\n");
            target_lane_id = ego_lane_id_;
            behavior_.lat_behavior = common::LateralBehavior::kLaneKeeping;
        }
        ...
        if (map_itf_->GetRightLaneId(ego_lane_id_, &target_lane_id) !=
            kSuccess) {
            printf(
                "[BP]Commanding a right lane change, but no existing lane.\n");
            target_lane_id = ego_lane_id_;
            behavior_.lat_behavior = common::LateralBehavior::kLaneKeeping;
        }
    ...
    reference_desired_velocity_ = std::floor(
        std::min(std::max(v_ref - 2.0, 0.0), user_desired_velocity_));
...
ErrorType
BehaviorPlanner::ConstructLaneFromSamples(const vec_E<Vecf<2>>& samples,
                                          Lane* lane) {
    double d = 0.0;
...
ErrorType
BehaviorPlanner::JudgeBehaviorByLaneId(const int ego_lane_id_by_pos,
                                       LateralBehavior* behavior_by_lane_id) {
...
ErrorType
BehaviorPlanner::UpdateEgoBehavior(const LateralBehavior& behavior_by_lane_id) {
...
ErrorType
BehaviorPlanner::GetPotentialLaneIds(const int source_lane_id,
