...
void PublishControl() {
    ...
    if (p_bp_server_ == nullptr)
        return;
    if (p_ctrl_input_smm_buff_ == nullptr)
        return;
    ...
    if (delta_t > 100.0 / fs_work_rate)
        delta_t = 1.0 / fs_work_rate;
    ...
    last_smm.GetLeadingVehicleOnLane(
        last_smm.ego_behavior().ref_lane, desired_state,
        last_smm.surrounding_vehicles(), lat_range, &leading_vehicle,
...
int main(int argc, char** argv) {
    ...
    // Declare bp
    p_bp_server_ =
        new planning::BehaviorPlannerServer(nh, bp_work_rate, ego_id);
