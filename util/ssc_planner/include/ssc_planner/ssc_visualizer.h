#ifndef _UTIL_SSC_PLANNER_INCLUDE_SSC_PLANNER_SSC_VISUALIZER_H_
#define _UTIL_SSC_PLANNER_INCLUDE_SSC_PLANNER_SSC_VISUALIZER_H_
...
class SscVisualizer {
  ...
  private:
    ...
    void VisualizeSurroundingVehicleTrajInSscSpace(
        ...
        const vec_E<std::unordered_map<int, vec_E<common::FsVehicle>>>&
            trajs_set,
        const SscMap* p_ssc_map);
    void VisualizeCorridorsInSscSpace(
        const ros::Time& stamp,
        const vec_E<common::DrivingCorridor> corridor_vec,
        const SscMap* p_ssc_map);
...
#endif // _UTIL_SSC_PLANNER_INCLUDE_SSC_PLANNER_SSC_VISUALIZER_H_