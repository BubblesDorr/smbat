...
ErrorType SscPlanner::RunOnce() {
    ...
    LOG(WARNING) << "[Ssc]construct ssc map and corridor time cost: "
                 << t_sscmap << " ms";
...
ErrorType SscPlanner::RunQpOptimization() {
    ...
    if (cube_list.size() != forward_behaviors_.size()) {
        LOG(ERROR) << "[Ssc]cube list " << static_cast<int>(cube_list.size())
                   ...
                   << ", forward traj "
                   << static_cast<int>(forward_trajs_.size()) << ", flag size "
                   << static_cast<int>(if_corridor_valid.size());
        return kWrongStatus;
    }
    ...
    for (int i = 0; i < static_cast<int>(cube_list.size()); i++) {
        ...
        end_constraints.push_back(Vecf<2>(
            std::max(fs_vehicle_traj[num_states - 1].frenet_state.vec_s[1],
                     cfg_.planner_cfg().velocity_singularity_eps()),
        ...
        for (int n = 0; n < num_states; n++) {
            ...
            ref_points.push_back(
                Vecf<2>(fs_vehicle_traj[n].frenet_state.vec_s[0],
        ...
        if (spline_generator.GetBezierSplineUsingCorridor(
            ...
            if (is_lateral_independent_) {
                ...
                for (auto& cube : cube_list[i]) {
                    LOG(ERROR)
                        << std::fixed << std::setprecision(3) << "[Ssc] t: ["
                ...
                for (int k = 0; k < ref_stamps.size(); ++k) {
                    LOG(ERROR) << std::fixed << std::setprecision(4) << "[Ssc]"
                               << k << " t: " << ref_stamps[k]
                               << ", x: " << ref_points[k].x()
                               << ", y: " << ref_points[k].y();
                }
                ...
                for (int k = 0; k < forward_trajs_[i].size(); ++k) {
                    ...
                    LOG(ERROR) << std::fixed << std::setprecision(4) << "[Ssc]"
                               << k << " t: " << v.state().time_stamp
                               << ", x: " << v.state().vec_position.x()
                               << ", y: " << v.state().vec_position.y()
                               << ", v: " << v.state().velocity;
                }
                LOG(ERROR) << "[Ssc]ref lane range: ["
                           << nav_lane_local_.begin() << ", "
                           << nav_lane_local_.end() << "]";

                LOG(ERROR) << std::fixed << std::setprecision(4)
                           << "[Ssc]Start sd velocity ("
                           << start_constraints[1](0) << ", "
                           << start_constraints[1](1) << ")";
                LOG(ERROR) << std::fixed << std::setprecision(4)
                           << "[Ssc]Start sd acceleration ("
                           << start_constraints[2](0) << ", "
        ...
        if (is_lateral_independent_ && !bezier_spline_gen_success)
            continue;
    ...
    for (int i = 1; i < num_cubes; i++) {
        if (cubes[i - 1].t_ub != cubes[i].t_lb) {
            ...
            LOG(ERROR) << "[Ssc]Err - t: [" << cubes[i - 1].t_lb << ", "
                       ...
                       << "]";
            LOG(ERROR) << "[Ssc]Err - t: [" << cubes[i].t_lb << ", "
                       << cubes[i].t_ub << "], x: [" << cubes[i].p_lb[0] << ", "
                       << cubes[i].p_ub[0] << "], y: [" << cubes[i].p_lb[1]
                       << ", " << cubes[i].p_ub[1] << "]";
            return kWrongStatus;
        }
    }
...
ErrorType SscPlanner::StateTransformForInputData() {
    ...
    // * Ego forward simulation trajs states and vertices
    {
        ...
        for (int i = 0; i < (int)forward_trajs_.size(); ++i) {
            ...
            for (int k = 0; k < (int)forward_trajs_[i].size(); ++k) {
                ...
                // vertices
                vec_E<Vec2f> v_vec;
                common::SemanticsUtils::GetVehicleVertices(ego_param,
                                                           traj_state, &v_vec);
                global_point_vec.insert(global_point_vec.end(), v_vec.begin(),
                                        v_vec.end());
            }
        ...
        for (int i = 0; i < surround_forward_trajs_.size(); ++i) {
            for (auto it = surround_forward_trajs_[i].begin();
                ...
                for (int k = 0; k < it->second.size(); ++k) {
                    ...
                    // vertices
                    vec_E<Vec2f> v_vec;
                    common::SemanticsUtils::GetVehicleVertices(
                        it->second[k].param(), traj_state, &v_vec);
                    global_point_vec.insert(global_point_vec.end(),
                                            v_vec.begin(), v_vec.end());
                }
            }
        }
    }
    ...
    // * Obstacle grids
    {
        for (auto it = obstacle_grids_.begin(); it != obstacle_grids_.end();
             ++it) {
            Vec2f pt((*it)[0], (*it)[1]);
    ...
    StateTransformSingleThread(global_state_vec, global_point_vec,
                               &frenet_state_vec, &fs_point_vec);
    LOG(WARNING) << "[Ssc]Single thread transform time cost: "
                 << timer_stf.toc() << " ms.";
#endif
    ...
    // * Ego vehicle state and vertices
    {
        ...
        for (int i = 0; i < num_v; ++i) {
            fs_ego_vehicle_.vertices.push_back(
                fs_point_vec[offset * num_v + i]);
        }
        offset++;
    }
    ...
    // * Ego forward simulation trajs states and vertices
    {
        ...
        if (forward_trajs_.size() < 1)
            return kWrongStatus;
        for (int j = 0; j < (int)forward_trajs_.size(); ++j) {
            if (forward_trajs_[j].size() < 1)
                assert(false);
    ...
    for (const auto t : t_vec_xy) {
        ...
        if (fabs(state.curvature) > 0.33) {
            LOG(ERROR) << "[Ssc][Validate]initial_state velocity "
                       << initial_state_.velocity << " Curvature "
                       << state.curvature << " invalid.";
            return kWrongStatus;
        }
    }
    return kSuccess;
