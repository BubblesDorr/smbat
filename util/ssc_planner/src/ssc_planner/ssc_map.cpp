...
ErrorType
SscMap::GetInitialCubeUsingSeed(const Vec3i& seed_0, const Vec3i& seed_1,
...
ErrorType
SscMap::ConstructSscMap(const std::unordered_map<int, vec_E<common::FsVehicle>>&
...
ErrorType SscMap::ConstructCorridorUsingInitialTrajectory(
    ...
    for (int i = 0; i < seed_num; ++i) {
        if (i == 0) {
            ...
            if (!CheckIfCubeIsFree(p_grid, cube)) {
                LOG(ERROR)
                    << "[Ssc] SccMap - Initial cube is not free, seed id: "
                    << i;
            ...
            if (CheckIfCubeContainsSeed(driving_corridor.cubes.back().cube,
                ...
                common::AxisAlignedCubeNd<int, 3> cube;
                GetInitialCubeUsingSeed(traj_seeds[i], traj_seeds[i + 1],
                                        &cube);
                ...
                std::array<bool, 6> dirs_disabled = {false, false, false,
                                                     false, false, false};
                InflateCubeIn3dGrid(p_grid, dirs_disabled,
                                    config_.inflate_steps, &cube);
                common::DrivingCube driving_cube;
                driving_cube.cube = cube;
                driving_cube.seeds.push_back(traj_seeds[i]);
                driving_corridor.cubes.push_back(driving_cube);
    ...
    if (is_valid) {
        ...
        // ~ Cut cube on time axis
        driving_corridor.cubes.back().cube.upper_bound[2] =
            traj_seeds.back()(2);
    ...
    for (int i = 0; i < num_cube - 1; ++i) {
        if (1) // ~ Enable s direction
        {
            ...
            if (abs(cube_0_ub - cube_1_lb) < margin[0]) {
                ...
                GetTimeCoveredCubeIndices(p_corridor, i + 1, 1, t_trans,
                                          &up_idx_list);
                // ~ Downward
                std::vector<int> down_idx_list;
                GetTimeCoveredCubeIndices(p_corridor, i, 0, t_trans,
                                          &down_idx_list);
                for (const auto& idx : up_idx_list) {
                    InflateCubeOnXNegAxis(p_grid, room,
                                          &(p_corridor->cubes[idx].cube));
                }
                for (const auto& idx : down_idx_list) {
                    InflateCubeOnXPosAxis(p_grid, room,
                                          &(p_corridor->cubes[idx].cube));
                }
            }
        ...
        if (1) // ~ Enable d direction
        {
            ...
            if (abs(cube_0_ub - cube_1_lb) < margin[1]) {
                ...
                GetTimeCoveredCubeIndices(p_corridor, i, 0, t_trans,
                                          &down_idx_list);
                for (const auto& idx : up_idx_list) {
                    InflateCubeOnYNegAxis(p_grid, room,
                                          &(p_corridor->cubes[idx].cube));
                }
                for (const auto& idx : down_idx_list) {
                    InflateCubeOnYPosAxis(p_grid, room,
                                          &(p_corridor->cubes[idx].cube));
                }
            }
...
bool SscMap::CheckIfCubeContainsSeed(
    ...
    for (int i = 0; i < 3; ++i) {
        if (cube_a.lower_bound[i] > seed(i) ||
            cube_a.upper_bound[i] < seed(i)) {
            return false;
        }
    }
    return true;
    ...
    for (const auto corridor : driving_corridor_vec_) {
        ...
        if (!corridor.is_valid) {
            ...
            for (int k = 0; k < static_cast<int>(corridor.cubes.size()); ++k) {
                ...
                if (k == 0) {
                    if (y_lb > initial_fs_.vec_dt[0] ||
                        y_ub < initial_fs_.vec_dt[0]) {
                        LOG(ERROR)
                            << "[Ssc] SscMap - Initial state out of bound d: "
...
ErrorType
SscMap::FillDynamicPart(const std::unordered_map<int, vec_E<common::FsVehicle>>&
                            sur_vehicle_trajs_fs) {
...
ErrorType
SscMap::FillMapWithFsVehicleTraj(const vec_E<common::FsVehicle> traj) {
    if (traj.size() == 0) {
