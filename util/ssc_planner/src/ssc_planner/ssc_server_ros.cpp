...
void SscPlannerServer::PublishData() {
    ...
    // trajectory feedback
    {
        ...
        if (use_sim_state_) {
            ...
            int num_cycles = std::floor(
                (current_time - executing_traj_->begin()) / plan_horizon);
            decimal_t ct = executing_traj_->begin() + num_cycles * plan_horizon;
            {
                ...
                if (executing_traj_->GetState(ct, &state) == kSuccess) {
                    ...
                    printf("[SscPlannerServer]cannot evaluate state at %lf "
                           "with begin "
                           "%lf.\n",
        ...
        // trajectory visualization
        {
            ...
            if (require_intervention_signal_) {
                ...
                executing_traj_->GetState(executing_traj_->begin(),
                                          &state_begin);
                Vec3f pos = Vec3f(state_begin.vec_position[0],
                                  state_begin.vec_position[1], 5.0);
                common::VisualizationUtil::
                    GetRosMarkerTextUsingPositionAndString(
                        pos, std::string("Intervention Needed!"),
                        common::cmap["red"], Vec3f(5.0, 5.0, 5.0), 0,
                        &traj_status);
...
ErrorType
SscPlannerServer::FilterSingularityState(const vec_E<common::State>& hist,
    ...
    if (fabs(filter_state->velocity) < singular_velocity &&
        ...
        printf("[SscPlannerServer]Detect singularity velocity %lf angle (%lf, "
               "%lf).\n",
...
void SscPlannerServer::PlanCycleCallback() {
    ...
    if (executing_traj_ == nullptr || !executing_traj_->IsValid() ||
        ...
        printf("[SscPlannerServer]init plan success with stamp: %lf and angle "
               "%lf.\n",
...
void SscPlannerServer::Replan() {
    ...
    printf("[SscPlannerServer]init stamp: %lf, plan horizon: %lf, num cycles "
           "%d.\n",
           global_init_stamp_, plan_horizon, num_cycles_ahead);
    printf("[SscPlannerServer]Replan at cur time %lf with executing traj begin "
    ...
    printf("[SscPlannerServer]t %lf, desired state (x,y,v,a,theta):(%lf, %lf, "
           "%lf, "
           "%lf, %lf).\n",
           t, desired_state.vec_position[0], desired_state.vec_position[1],
           desired_state.velocity, desired_state.acceleration,
           desired_state.angle);
