...
ErrorType
EudmPlannerMapAdapter::GetEgoLaneIdByPosition(const std::vector<int>& navi_path,
                                              int* lane_id) {
    if (!is_valid_) {
...
ErrorType EudmPlannerMapAdapter::GetChildLaneIds(const int lane_id,
    ...
    if (it == p_semantic_lane_set->semantic_lanes.end()) {
        ...
        child_ids->assign(it->second.child_id.begin(),
                          it->second.child_id.end());
    }
    return kSuccess;
}
...
bool EudmPlannerMapAdapter::IsLaneConsistent(const int lane_id_old,
    ...
    if (visited_set.find(lane_id_new) != visited_set.end())
        return true;
    return false;
}

ErrorType
EudmPlannerMapAdapter::GetFatherLaneIds(const int lane_id,
...
ErrorType EudmPlannerMapAdapter::GetLeadingVehicleOnLane(
    ...
    if (!is_valid_)
        return kWrongStatus;
    if (map_->GetLeadingVehicleOnLane(ref_lane, ref_state, vehicle_set,
...
EudmPlannerMapAdapter::GetLeadingAndFollowingVehiclesFrenetStateOnLane(
    ...
    if (map_->GetLeadingAndFollowingVehiclesFrenetStateOnLane(
            ...
            leading_vehicle, leading_fs, has_following_vehicle,
            following_vehicle, following_fs) != kSuccess) {
        return kWrongStatus;
    }
    return kSuccess;
}
...
ErrorType
EudmPlannerMapAdapter::GetKeyVehicles(common::VehicleSet* key_vehicle_set) {
    if (!is_valid_)
        return kWrongStatus;
...
ErrorType EudmPlannerMapAdapter::CheckCollisionUsingState(
    ...
    if (!is_valid_)
        return kWrongStatus;
    if (map_->CheckCollisionUsingState(param_a, state_a, param_b, state_b,
                                       res) != kSuccess) {
        return kWrongStatus;
    }
    return kSuccess;
