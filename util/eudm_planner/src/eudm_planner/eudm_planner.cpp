...
ErrorType EudmPlanner::GetSurroundingForwardSimAgents(
    ...
    for (const auto& psv : surrounding_semantic_vehicles.semantic_vehicles) {
        ...
        if (state.acceleration >= 0) {
            ...
            decimal_t est_vel = std::max(
                0.0, state.velocity + state.acceleration * sim_time_total_);
...
ErrorType EudmPlanner::RunEudm() {
    ...
    for (int i = 0; i < n_sequence; ++i) {
        thread_set[i] = std::thread(&EudmPlanner::SimulateActionSequence, this,
                                    ego_vehicle_, surrounding_fsagents,
                                    action_script[i], i);
    }
    for (int i = 0; i < n_sequence; ++i) {
        thread_set[i].join();
    ...
    for (int i = 0; i < n_sequence; ++i) {
        ...
        line_info << "[s:" << sim_res_[i] << "|r:" << risky_res_[i]
                  << "|c:" << std::fixed << std::setprecision(3)
                  << final_cost_[i] << "]";
        ...
        if (sim_res_[i]) {
            ...
            for (const auto& c : progress_cost_[i]) {
                line_info << std::fixed << std::setprecision(2)
                          ...
                          << c.safety.rss << "_" << c.safety.occu_lane << ";"
                          << c.navigation.lane_change_preference << ";"
                          << c.weight;
                line_info << "|";
            }
            line_info << "]";
    ...
    // * evaluate
    if (EvaluateMultiThreadSimResults(&winner_id_, &winner_score_) !=
        kSuccess) {
        LOG(ERROR)
...
ErrorType
EudmPlanner::UpdateSimSetupForScenario(const std::vector<DcpAction>& action_seq,
    ...
    case DcpLonAction::kMaintain: {
        idm_param_tmp.kDesiredVelocity =
            std::min(desired_vel, desired_velocity_);
        break;
    }
    default: {
...
ErrorType
EudmPlanner::UpdateSimSetupForLayer(const DcpAction& action,
    ...
    decimal_t forward_lane_len = std::min(
        std::max(state.velocity * cfg_.sim().ref_line().len_vel_coeff(),
    ...
    if (ego_fsagent->lat_behavior != LateralBehavior::kLaneKeeping) {
        ...
        map_itf_->GetLeadingAndFollowingVehiclesFrenetStateOnLane(
            ...
            &front_vehicle, &front_fs, &has_rear_vehicle, &rear_vehicle,
            &rear_fs);

        ego_fsagent->target_gap_ids(0) =
            has_front_vehicle ? front_vehicle.id() : -1;
        ego_fsagent->target_gap_ids(1) =
            has_rear_vehicle ? rear_vehicle.id() : -1;
        ...
        if (cfg_.safety().rss_for_layers_enable()) {
            ...
            decimal_t s_ego_fbumper =
                ego_fs.vec_s[0] + ego_fsagent->vehicle.param().length() / 2.0 +
                ego_fsagent->vehicle.param().d_cr();
            decimal_t s_ego_rbumper =
                ego_fs.vec_s[0] - ego_fsagent->vehicle.param().length() / 2.0 +
                ego_fsagent->vehicle.param().d_cr();

            if (has_front_vehicle) {
                decimal_t s_front_rbumper =
                    front_fs.vec_s[0] - front_vehicle.param().length() / 2.0 +
...
ErrorType EudmPlanner::SimulateScenario(
    ...
    vec_E<std::unordered_map<int, vec_E<common::Vehicle>>>*
        sub_surround_trajs) {
    // * declare variables which will be used to track traces from multiple
    // layers
    ...
    for (const auto& p_fsa : surrounding_fsagents.forward_sim_agents) {
        surround_trajs_multilayers.insert(
            std::pair<int, vec_E<common::Vehicle>>(
    ...
    for (int i = 0; i < static_cast<int>(action_seq_sim.size()); ++i) {
        ...
        // For each action, the context info will not change, so we can use it
        // in every step. A low-level reactive lane-changing controller can be
        ...
        for (auto it =
                 surrounding_fsagents_this_layer.forward_sim_agents.begin();
             it != surrounding_fsagents_this_layer.forward_sim_agents.end();
             ++it) {
        ...
        // LOG(INFO) << "[RssTime]safety check time per action: " <<
        // timer.toc();
        ...
        for (const auto& v :
             surrounding_fsagents_this_layer.forward_sim_agents) {
            int id = v.first;
        ...
        CostFunction(action_this_layer, ego_fsagent_this_layer,
                     ...
                     surround_trajs_multisteps, verbose, &cost,
                     &is_risky_action, &risky_ids);
        if (is_risky_action) {
...
ErrorType EudmPlanner::UpdateLateralActionSequence(
    ...
    case DcpLatAction::kLaneChangeLeft: {
        for (int i = cur_idx + 1; i < static_cast<int>(action_seq->size());
             ++i) {
...
ErrorType EudmPlanner::RunOnce() {
    ...
    LOG(WARNING) << "[Eudm][Setup]Desired vel:" << desired_velocity_
                 ...
                 << lc_info_.left_solid_lane << ","
                 << lc_info_.right_solid_lane;
    ...
    line_info << "] cost: " << std::fixed << std::setprecision(3)
              << winner_score_ << " time cost: " << timer.toc() << " ms.";
    LOG(WARNING) << line_info.str();
...
ErrorType EudmPlanner::EvaluateSafetyStatus(
    ...
    for (int i = 0; i < num_states; i += check_per_state) {
        ...
        common::RssChecker::RssCheck(traj_a[i], traj_b[i], rss_stf_,
                                     rss_config_, &is_rss_safe, &type,
                                     &rss_vel_low, &rss_vel_up);
        if (!is_rss_safe) {
            ret_is_rss_safe = false;
            *risky_id = traj_b.size() ? traj_b[0].id() : 0;
            if (cfg_.cost().safety().rss_cost_enable()) {
                if (type ==
                    common::RssChecker::LongitudinalViolateType::TooFast) {
                    cost_tmp +=
                        cfg_.cost().safety().rss_over_speed_linear_coeff() *
                        traj_a[i].state().velocity *
                        pow(10,
                            cfg_.cost().safety().rss_over_speed_power_coeff() *
                                fabs(traj_a[i].state().velocity - rss_vel_up));
                } else if (type == common::RssChecker::LongitudinalViolateType::
                                       TooSlow) {
                    cost_tmp +=
                        cfg_.cost().safety().rss_lack_speed_linear_coeff() *
                        traj_a[i].state().velocity *
                        pow(10,
                            cfg_.cost().safety().rss_lack_speed_power_coeff() *
...
ErrorType EudmPlanner::StrictSafetyCheck(
    ...
    for (auto it = surround_trajs.begin(); it != surround_trajs.end(); it++) {
        ...
        if (num_points_other != num_points_ego) {
            *is_safe = false;
            LOG(ERROR)
                << "[Eudm]unsafe due to incomplete sim record for vehicle";
            return kSuccess;
        }
        for (int i = 0; i < num_points_ego; i++) {
            ...
            map_itf_->CheckCollisionUsingState(
                inflated_a.param(), inflated_a.state(), inflated_b.param(),
                inflated_b.state(), &is_collision);
            if (is_collision) {
                *is_safe = false;
...
ErrorType EudmPlanner::CostFunction(
    ...
    if (map_itf_->GetLeadingVehicleOnLane(
        ...
        if (ego_fsagent.vehicle.state().velocity < desired_velocity_ &&
            ...
            cost_tmp.efficiency.leading_to_desired_vel =
                ...
                (cfg_.cost()
                         .effciency()
                         .ego_speed_blocked_by_leading_unit_cost() *
    ...
    for (const auto& surround_traj : surround_trajs) {
        ...
        bool is_safe = true;
        int risky_id = 0;
        EvaluateSafetyStatus(ego_traj, surround_traj.second, &safety_cost,
                             &is_safe, &risky_id);
        if (!is_safe) {
    ...
    if (seq_lat_behavior == LateralBehavior::kLaneChangeLeft ||
        ...
        if (is_cancel_behavior) {
            cost_tmp.navigation.lane_change_preference =
                std::max(
                    cfg_.cost().navigation().lane_change_unit_cost_vel_lb(),
                    ego_velocity) *
            ...
            if (lc_info_.recommend_lc_left &&
                ...
                cost_tmp.navigation.lane_change_preference =
                    ...
                    cfg_.cost()
                        .navigation()
                        .lane_change_left_recommendation_reward();
                if (action.lat != DcpLatAction::kLaneChangeLeft) {
                    cost_tmp.navigation.lane_change_preference +=
                        std::max(cfg_.cost()
                                     .navigation()
                                     .lane_change_unit_cost_vel_lb(),
                                 ego_velocity) *
                ...
                cost_tmp.navigation.lane_change_preference =
                    ...
                    cfg_.cost()
                        .navigation()
                        .lane_change_right_recommendation_reward();
                if (action.lat != DcpLatAction::kLaneChangeRight) {
                    cost_tmp.navigation.lane_change_preference +=
                        std::max(cfg_.cost()
                                     .navigation()
    ...
    for (int i = 0; i < static_cast<int>(dt_steps.size()); i++) {
        ...
        // * For surrounding agents
        {
            for (const auto& p_fsa :
                 surrounding_fsagents_this_step.forward_sim_agents) {
                all_sim_vehicles.vehicles.at(p_fsa.first)
                    .set_id(kInvalidAgentId);
...
ErrorType EudmPlanner::SurroundingAgentForwardSim(
    ...
    common::State state_output;
    decimal_t distance_residual_ratio = 0.0;
    map_itf_->GetLeadingVehicleOnLane(
        fsagent.lane, fsagent.vehicle.state(), all_sim_vehicles,
        fsagent.lat_range, &leading_vehicle, &distance_residual_ratio);
...
ErrorType
EudmPlanner::EgoAgentForwardSim(const ForwardSimEgoAgent& ego_fsagent,
    ...
    if (ego_fsagent.lat_behavior == LateralBehavior::kLaneKeeping) {
        ...
        if (map_itf_->GetLeadingVehicleOnLane(
            ...
            map_itf_->CheckCollisionUsingState(
                ...
                leading_vehicle.param(), leading_vehicle.state(),
                &is_collision);
            if (is_collision) {
                return kWrongStatus;
            }
        ...
        if (map_itf_->GetLeadingVehicleOnLane(
                ...
                all_sim_vehicles, ego_fsagent.lat_range,
                &current_leading_vehicle,
                &distance_residual_ratio) == kSuccess) {
            // ~ with leading vehicle
            bool is_collision = false;
            map_itf_->CheckCollisionUsingState(
                ego_fsagent.vehicle.param(), ego_fsagent.vehicle.state(),
                current_leading_vehicle.param(),
                current_leading_vehicle.state(), &is_collision);
            if (is_collision) {
                return kWrongStatus;
            }
        }
        ...
        if (gap_rear_vehicle.id() != kInvalidAgentId &&
            ...
            if (ego_fsagent.target_stf.GetFrenetStateFromState(
                    ego_fsagent.vehicle.state(), &ego_on_tarlane_fs) ==
                    kSuccess &&
                ego_fsagent.target_stf.GetFrenetStateFromState(
                    gap_rear_vehicle.state(), &rear_on_tarlane_fs) ==
                    kSuccess) {
                ...
                common::RssChecker::RssCheck(
                    ego_fsagent.vehicle, gap_rear_vehicle,
                    ego_fsagent.target_stf, rss_config_strict_as_rear_,
                ...
                if (!is_rss_safe) {
                    if (type ==
                        ...
                        sim_param.idm_param.kDesiredVelocity = std::max(
                            ...
                            rss_vel_low +
                                cfg_.sim().ego().evasive().lon_extraspeed());
                        ...
                        sim_param.max_lon_acc_jerk =
                            cfg_.sim().ego().evasive().lon_jerk();
                    }
                }
                if (cfg_.sim().ego().evasive().virtual_barrier_enable()) {
                    if (ego_on_tarlane_fs.vec_s[0] -
                            rear_on_tarlane_fs.vec_s[0] <
                    ...
                    if (gap_front_vehicle.id() != kInvalidAgentId &&
                        ego_fsagent.target_stf.GetFrenetStateFromState(
                            ...
                            kSuccess) {
                        if (front_on_tarlane_fs.vec_s[0] -
                                ego_on_tarlane_fs.vec_s[0] <
...
ErrorType
EudmPlanner::JudgeBehaviorByLaneId(const int ego_lane_id_by_pos,
...
ErrorType
EudmPlanner::GetPotentialLaneIds(const int source_lane_id,
