...
bool EudmManager::IsTriggerAppropriate(const LateralBehavior& lat) {
    ...
    for (int i = 0; i < num_action_seqs; i++) {
        if (!last_snapshot_.sim_res[i] || last_snapshot_.risky_res[i])
            continue;
    ...
    if (num_match_seqs < kMinMatchSeqs)
        return false;
    return true;
}
...
ErrorType EudmManager::Prepare(
...

    {
        ...
        line_info << "[Eudm][Manager]Replan context <valid, stamp, seq>:<"
                  << context_.is_valid << "," << std::fixed
                  << std::setprecision(3) << context_.seq_start_time << ",";
...
ErrorType
EudmManager::GenerateLaneChangeProposal(const decimal_t& stamp,
                                        const planning::eudm::Task& task) {
    if (!bp_.cfg().function().active_lc_enable()) {
    ...
    if (stamp - last_lc_proposal_.trigger_time <
        ...
        preliminary_active_requests_.clear();
        LOG(WARNING) << std::fixed << std::setprecision(5)
                     << "[Eudm][ActiveLc]Clear request due to cold down:"
    ...
    if (last_snapshot_.plan_state.velocity <
        ...
        LOG(WARNING) << std::fixed << std::setprecision(5)
                     ...
                     << last_snapshot_.plan_state.velocity << " at time "
                     << stamp;
        return kSuccess;
    }
    ...
    if (preliminary_active_requests_.empty()) {
        ...
        LOG(WARNING) << std::fixed << std::setprecision(5)
                     << "[Eudm][ActiveLc]trigger time:"
                     << this_request.trigger_time << " Init requesting "
        ...
        LOG(WARNING) << std::fixed << std::setprecision(5)
                     << "[Eudm][ActiveLc]trigger time:"
                     << this_request.trigger_time << " Consequent requesting "
                     << common::SemanticsUtils::RetLatBehaviorName(
                            this_request.lat)
                     << " at " << this_request.desired_operation_time
                     << " with lane id " << this_request.ego_lane_id;
        ...
        if (last_request.ego_lane_id != this_request.ego_lane_id) {
            LOG(WARNING) << "[Eudm][ActiveLc]Invalid this request due to lane "
                            "id inconsitent.";
            preliminary_active_requests_.clear();
            return kSuccess;
        }
        if (last_request.lat != this_request.lat) {
            LOG(WARNING)
                << "[Eudm][ActiveLc]Invalid this request due to behavior "
                   "inconsitent.";
            preliminary_active_requests_.clear();
            return kSuccess;
        }
        if (fabs(last_request.desired_operation_time -
            ...
            bp_.cfg()
                .function()
                .active_lc()
                .consistent_operate_time_min_gap()) {
            LOG(WARNING) << "[Eudm][ActiveLc]Invalid this request due to time "
                            "inconsitent.";
            preliminary_active_requests_.clear();
            return kSuccess;
        }
        ...
        LOG(WARNING) << "[Eudm][ActiveLc]valid this request. Queue size "
                     << preliminary_active_requests_.size()
                     << " and operate at " << operation_at_seconds;
    }

    if (preliminary_active_requests_.size() >=
        bp_.cfg().function().active_lc().consistent_min_num_frame()) {
        if (operation_at_seconds < bp_.cfg()
                                           .function()
                                           .active_lc()
                                           .activate_max_duration_in_seconds() +
                                       kEPS) {
            ...
            preliminary_active_requests_.clear();
            // LOG(WARNING) << "[HMI]Abandan Queue due to change time not
            // legal.";
        }
    }
    ...
    if (task.is_under_ctrl) {
        if (!lc_context_.completed) {
            if (!map_adapter_.IsLaneConsistent(lc_context_.ego_lane_id,
                ...
                LOG(WARNING)
                    << "[HMI]lane change completed due to different lane id "
                ...
                if (task.user_perferred_behavior != 1 &&
                    ...
                    if (bp_.cfg()
                        ...
                        if (lc_context_.lat ==
                            ...
                            LOG(WARNING)
                                << "[HMI]ACTIVE [Left] auto cancel due to "
                                   "outdated for "
                                << stamp - lc_context_.desired_operation_time
                                << " s. Cd alc.";
                        } else {
                            LOG(WARNING)
                                << "[HMI]ACTIVE [Right] auto cancel due to "
                                   "outdated for "
                                << stamp - lc_context_.desired_operation_time
                                << " s. Cd alc.";
                        }
                        ...
                        last_lc_proposal_.trigger_time = stamp;
                               ...
                               lc_context_.lat ==
                                   LateralBehavior::kLaneChangeLeft) {
                        LOG(WARNING)
                            << "[HMI]ACTIVE [Left] canceled due to forbidden "
                               "signal. Cd alc.";
                        lc_context_.completed = true;
                        lc_context_.trigger_when_appropriate = false;
                        last_lc_proposal_.trigger_time = stamp;
                        ...
                        LOG(WARNING) << "[HMI]ACTIVE [Right] canceled due to "
                                        "forbidden signal. "
                                        "Cd alc.";
                        lc_context_.completed = true;
                        lc_context_.trigger_when_appropriate = false;
                        ...
                        LOG(WARNING) << "[HMI]ACTIVE [left] canceled due to "
                                        "human opposite signal. "
                                        "Cd alc.";
                        lc_context_.completed = true;
                        lc_context_.trigger_when_appropriate = false;
                        last_lc_proposal_.trigger_time = stamp;
                        ...
                        LOG(WARNING) << "[HMI]ACTIVE canceled due to human "
                                        "active signal. "
                                        "Cd alc.";
                        lc_context_.completed = true;
                        lc_context_.trigger_when_appropriate = false;
            ...
            if (task.user_perferred_behavior != 1 &&
                ...
                LOG(WARNING)
                    << "[HMI]clear cached stick trigger state. Cd alc.";
                lc_context_.trigger_when_appropriate = false;
                last_lc_proposal_.trigger_time = stamp;
            } else if (task.user_perferred_behavior != -1 &&
                       last_task_.user_perferred_behavior == -1 &&
            ...
            if (task.user_perferred_behavior == 1 &&
                last_task_.user_perferred_behavior != 1) {
                // receive a lane change right trigger and previous action has
                // been completed
                if (task.lc_info.forbid_lane_change_right) {
                    LOG(WARNING) << "[HMI]cannot stick [Right]. Will trigger "
                                    "when appropriate.";
                    lc_context_.trigger_when_appropriate = true;
                    lc_context_.lat = LateralBehavior::kLaneChangeRight;
                } else {
                    if (IsTriggerAppropriate(
                            LateralBehavior::kLaneChangeRight)) {
                        ...
                        LOG(WARNING) << std::fixed << std::setprecision(5)
                                     ...
                                     << last_task_.user_perferred_behavior
                                     << "->" << task.user_perferred_behavior
                                     << " but not a good time. Will trigger "
                                        "when appropriate.";
                    }
                }
            } else if (task.user_perferred_behavior == -1 &&
                       last_task_.user_perferred_behavior != -1) {
                if (task.lc_info.forbid_lane_change_left) {
                    LOG(WARNING) << "[HMI]cannot stick [Left]. Will trigger "
                                    "when appropriate.";
                    lc_context_.trigger_when_appropriate = true;
...
ErrorType
EudmManager::EvaluateReferenceVelocity(const planning::eudm::Task& task,
                                       decimal_t* ref_vel) {
    ...
    *ref_vel =
        std::floor(std::min(std::max(v_ref, 0.0), task.user_desired_vel));
...
ErrorType EudmManager::Run(
    ...
    LOG(WARNING) << std::fixed << std::setprecision(4)
                 << "[Eudm]Sum & Reselect time cost " << t_sum_reselect
                 << " ms";
    ...
    if (map_adapter_.map()->GetRefLaneForStateByBehavior(
            ...
            snapshot.forward_lat_behaviors[snapshot.processed_winner_id]
                .front(),
