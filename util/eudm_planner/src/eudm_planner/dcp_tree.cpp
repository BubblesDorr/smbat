...
std::vector<DcpTree::DcpAction>
DcpTree::AppendActionSequence(const std::vector<DcpAction>& seq_in,
...
ErrorType DcpTree::GenerateActionScript() {
    ...
    for (int lon = 0; lon < static_cast<int>(DcpLonAction::MAX_COUNT); lon++) {
        ...
        ongoing_action_seq.push_back(DcpAction(
            DcpLonAction(lon), ongoing_action_.lat, ongoing_action_.t));
            ...
            for (int lat = 0; lat < static_cast<int>(DcpLatAction::MAX_COUNT);
                ...
                if (lat != static_cast<int>(ongoing_action_.lat)) {
                    auto actions = AppendActionSequence(
                        ongoing_action_seq,
                        DcpAction(DcpLonAction(lon), DcpLatAction(lat),
                                  layer_time_),
                        tree_height_ - h);
