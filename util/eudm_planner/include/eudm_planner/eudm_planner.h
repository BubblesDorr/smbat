#ifndef _UTIL_EUDM_PLANNER_INCLUDE_EUDM_PLANNER_EUDM_PLANNER_H_
#define _UTIL_EUDM_PLANNER_INCLUDE_EUDM_PLANNER_EUDM_PLANNER_H_
...
class EudmPlanner : public Planner {
  public:
    ...
    struct ForwardSimEgoAgent {
        ...
        common::LateralBehavior lat_behavior_longterm{
            LateralBehavior::kUndefined};
        ...
        common::LongitudinalBehavior lon_behavior{
            LongitudinalBehavior::kMaintain};
        ...
        Vec2i target_gap_ids; // optional, gap ids are fixed in each layer, but
                              // gap is changing in each step
    ...
    struct CostStructure {
        ...
        decimal_t ave() const {
            return (efficiency.ave() + safety.ave() + navigation.ave()) *
                   weight;
        }
        ...
        friend std::ostream& operator<<(std::ostream& os,
            ...
            os << "(efficiency: "
               << "ego (" << cost.efficiency.ego_to_desired_vel
               << ") + leading (" << cost.efficiency.leading_to_desired_vel
               << "), safety: (" << cost.safety.rss << ","
               << cost.safety.occu_lane
  ...
  private:
    ...
    ErrorType
    TranslateDcpActionToLonLatBehavior(const DcpAction& action,
                                       LateralBehavior* lat,
    ...
    ErrorType
    SimulateActionSequence(const common::Vehicle& ego_vehicle,
    ...
    ErrorType EvaluateSafetyStatus(const vec_E<common::Vehicle>& traj_a,
                                   ...
                                   int* risky_id);
    ErrorType
    EvaluateSinglePolicyTrajs(const std::vector<CostStructure>& progress_cost,
                              const CostStructure& tail_cost,
                              const std::vector<DcpAction>& action_seq,
                              decimal_t* score);
    ...
    ErrorType
    UpdateSimSetupForScenario(const std::vector<DcpAction>& action_seq,
    ...
    ErrorType
    UpdateEgoBehaviorsUsingAction(const DcpAction& action,
                                  ForwardSimEgoAgent* ego_fsagent) const;
    ...
    ErrorType
    UpdateLateralActionSequence(const int cur_idx,
    ...
    ErrorType
    SurroundingAgentForwardSim(const ForwardSimAgent& fsagent,
                               const common::VehicleSet& all_sim_vehicles,
                               const decimal_t& sim_time_step,
                               common::State* state_out) const;
...
#endif // _UTIL_EUDM_PLANNER_INCLUDE_EUDM_PLANNER_EUDM_PLANNER_H_